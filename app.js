const getFirstValue = array => array[1];

const convertMinsToSecs = minutes => minutes *= 60; // returns number of seconds
...
const lessThan100 = (num1, num2) => {
		...
		document.getElementById('output1').innerHTML = `Second value: ${getFirstValue(array)}`;
}

		function showConvertedMinsToSecs() {
		const minutes = parseInt(document.getElementById('minutesInput').value);
		document.getElementById('output2').innerHTML = `${minutes} minutes is equal to ${convertMinsToSecs(minutes)} seconds`;
}
    ...
    return array[0];
								...
								break;
						case 'convertMinsToSecs':
								code = `const convertMinsToSecs = (minutes) => {
    return minutes *= 60;
};`;
								break;
						case 'lessThan100':
